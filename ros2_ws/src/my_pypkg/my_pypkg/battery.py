...
class BatteryNode(Node):
    ...
    def check_battery_state(self):
        ...
        if self.battery_state_ == "full":
            if time_now - self.last_time_battery_state_changed_ > 4.0:
                ...
                self.get_logger().info("Batterry is empty! Charging ...")
            ...
            if time_now - self.last_time_battery_state_changed_ > 6.0:
                self.battery_state_ = "full"
                self.get_logger().info("Batterry is now full again!")
                self.last_time_battery_state_changed_ = time_now
    ...
    def call_set_led_server(self, led_number, led_state):
        ...
        future.add_done_callback(partial(self.callback_call_setled
                                         , led_number = led_number
                                         , led_state = led_state))

    def callback_call_setled(self, future, led_number, led_state):
        try:
